...
def move_player(key):
    global idx, tmr, pl_x, pl_y, pl_d, pl_a, pl_life, food, potion, blazegem, treasure, floor ,pl_str
    
    if dungeon[pl_y][pl_x] == 1:
        dungeon[pl_y][pl_x] = 0
        treasure = random.choice([0,0,0,1,1,1,1,1,1,2,6])
        if floor >= 10:
            treasure = random.choice([0,0,0,1,1,1,1,1,1,2,5,6])
            r = random.randint(0, 99)
            if treasure in (5, 6) and r < 50:
                treasure = 0
        if treasure == 0:
            potion = potion + 1
        if treasure == 1:
...
def draw_para(bg, fnt):
    ...
    draw_text(bg, f"Def Buff: {pl_def}", X+400, Y+6, fnt, WHITE)
    draw_text(bg, f"Pill: {def_pill}", X+400, Y+33, fnt, WHITE)
...
def main():
    ...
    global pl_a, pl_lifemax, pl_life, pl_str, food, potion, blazegem, pl_lv
    global pl_def, def_pill
    ...
    while True:
        ...
        if idx == 0:
            ...
            if tmr == 6:
                pl_def = 5 + random.randint(1, 5)
                def_pill = def_pill - 1
                set_message("Def up!(+" + str(pl_def) + ")")
            if tmr == 20:
                idx = 13
                tmr = 0
